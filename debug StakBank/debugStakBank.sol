...
contract StakBank is Ownable {
    ...
    function unstakeWithId(uint idStake) public {
        ...
        if (detail.isOldCoin) {
            ...
            console.log("       main reward: %s | first reward: %s", (_cummEth.sub(detail.lastWithdraw)).mul(detail.amount), detail.firstReward);
        }
    ...
    function rewardDistribution() public onlyOwner {
        ...
        if (totalTime > 0) {
            ...
            for(uint i = 0; i < stakingTrans.length; i++) {
                ...
                console.log("unitvalue: %s | firsttime: %s | coin: %s", unitValue, firstTime, transaction.amount);
            }
